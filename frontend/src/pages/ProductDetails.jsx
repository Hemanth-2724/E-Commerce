...
export default function ProductDetails() {
  ...
  const [authModal, setAuthModal]         = useState(false);   // show/hide
  const [authModalAction, setAuthModalAction] = useState("add to cart"); // label
  ...
  useEffect(() => {
    fetch(`${BASE_URL}/products?id=${id}`)
      ...
      .catch(() => {
        setProduct({
          productId: id, productName: "Premium Product", price: 2999,
          discountPercent: 10,
          description: "An incredible premium product with top-tier craftsmanship and high-quality materials. Designed for the modern individual who appreciates style and comfort.",
  ...
  const handleViewCart = (e) => {
    if (!user) {
      e.preventDefault();
      setAuthModalAction("view cart");
      setAuthModal(true);
    }
    /* if logged in, the Link navigates normally */
  };
  ...
  const images = [pImage, product.image2Url || pImage, product.image3Url || pImage];

  const handleScroll = (e) => {
    const newIndex = Math.round(e.target.scrollLeft / (e.target.offsetWidth || 1));
        ...
        <div className="detail-layout">
          ...
          <div style={{ position: "relative", overflow: "hidden" }}>
            ...
            <div className="gallery-indicator">
              {images.map((_, i) => (
                <div key={i} className={`dot ${i === activeImageIndex ? "active" : ""}`} />
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.5rem" }}>
              <span style={{ fontSize: "0.82rem", color: "var(--text-2)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>
                Quantity
              </span>
            ...
            <div style={{ display: "flex", gap: "1rem" }}>
              ...
              {/* View Cart — triggers modal if not logged in */}
              <Link
                to="/cart"
